...
because the plot is not in front of you.
[`auc()`](https://evalclass.github.io/precrec/reference/auc.md) reports
it in the `baselines` column:
...
The same model on the two datasets above. The area falls, and so does
the baseline it is read against - from 0.5 to 0.2. How much of the fall
is the classifier doing worse and how much is the class balance moving
...
work is picking a point on it, and the criterion that picks it is
...
of what it is at the one `mcc` picks. `fscore` and `mcc` are computed
from precision as well, so the prevalence is in the number they are
maximizing.

That does not make `mcc` the right answer either. It buys its precision
with sensitivity, and whether that trade is the one you want is a
question about the application, not about the data. What the table
settles is narrower and more useful: **the criterion is a choice**, and
on imbalanced data the prevalence-blind ones and the prevalence-aware
ones do not land anywhere near each other.

One caveat that applies whatever the balance: a threshold chosen on the
same data the model was evaluated on is optimistic, by however much the
criterion was free to chase. If the number is going to be quoted, choose
it on data the model has not seen.
