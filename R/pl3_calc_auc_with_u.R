...
#
# Create a placeholder AUC for a dataset that holds a single class
#
# See .create_na_curves(): the same idea, for `mode = "aucroc"`.
#
.create_na_uauc <- function(sdat) {
  s3obj <- structure(list(auc = NA_real_, ustat = NA_real_), class = "uauc")
...
.validate.uauc <- function(x) {
  ...
  # AUC. A placeholder result for a single-class dataset has none.
  auc <- x[["auc"]]
  .assert_internal(
    is.numeric(auc),
    length(auc) == 1L
