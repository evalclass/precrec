...
#'
#' @param x An `S3` object generated by [evalmod()].
...
#'
#' @param y Equivalent with `curvetype`.
#'
...
#' }
#'
#' @return The `plot` function shows a plot and returns NULL.
#'
...
#
# Check partial match - Basic evaluation metrics
#
...
#
# Check partial match - the metrics added for ROCR parity
#
# Tried last, after every metric this package has always had, so that a
...
.set_layout <- function(ctype_len, show_legend) {
  ...
  # cells empty, which is what happens for e.g. five metrics in two rows.
...
.get_xlim <- function(obj, curvetype) {
  if (curvetype == "rocs" || curvetype == "prcs") {
    ...
    # Every basic metric is drawn against the normalized rank
    xlim <- c(0, 1)
  }
...
#
# matplot has no "read it off the data" limit, so an unbounded metric gets
