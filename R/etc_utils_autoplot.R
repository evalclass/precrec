...
#'
#' The `autoplot` function plots performance evaluation metrics
#'   by using \pkg{ggplot2} instead of the general R plot.
#'
#' @param object An `S3` object generated by [evalmod()].
...
#'     `autoplot` returns a `grob` object. The `grob` object
#'     is internally generated by [gridExtra::arrangeGrob()].
#'     The [grid::grid.draw()] function takes a `grob` object and
...
#' }
#'
#' @return The `autoplot` function returns a `ggplot` object
#'   for a single-panel plot and a frame-grob object for a multiple-panel plot.
#'
#' @seealso [evalmod()] for generating an `S3` object.
#'   [fortify()] for converting a curves and points object
#'   to a data frame.  [plot()] for plotting the equivalent curves
...
#
# Both axes are metrics, so each takes the range its own metric needs. A
# metric that is unbounded above gets NULL, which is ggplot2 reading the
