...
.pl_main_rocprc <- function(mdat, model_type, dataset_type, class_name_pf,
  ...
  # Summarize average precision
  aps <- .gather_aps(lcurves, mdat)
...
#
# Get the average precision of every precision-recall curve
#
# One row per model and dataset, unlike the AUC table which carries a row per
# curve type: average precision is defined on the precision-recall curve
# only, so a ROC row would be a column of NA.
#
.gather_aps <- function(lcurves, mdat) {
  modnames <- attr(mdat, "data_info")[["modnames"]]
  dsids <- attr(mdat, "data_info")[["dsids"]]

  ap_vals <- rep(NA_real_, length(modnames))
  for (i in seq_along(lcurves)) {
    ap_vals[i] <- attr(lcurves[[i]][["prc"]], "ap")
  }
...
.validate_curves_common <- function(curves, class_name) {
  ...
  attr_names <- c(
    "aucs", "aps", "grp_avg", "data_info", "uniq_modnames",
