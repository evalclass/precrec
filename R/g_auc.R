...
#'
#' @section The baseline is an asymptote:
#'
...
#'   balance, and [average_precision()] shows the same thing, so it is not
#'   an artifact of the interpolation.
#'
#'   The example above stands: `0.129` against a baseline of `0.02` is a
#'   real result. But when the margin is small and the positives are few,
#'   [auc_boot()] puts an interval around the area from a single test set,
#'   and [auc_ci()] does it from several. Both now report the baseline
#'   beside the interval, which is the comparison worth making.
#'
...
#'   the area under the precision-recall curve. [auc_boot()] and [auc_ci()]
