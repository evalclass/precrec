...
#' \code{cmats} object and returns an \code{evals} object.
#' \code{evals} contains basic evaluation measures that are used
...
#'   \code{\link{create_confmats}}.
#' @return \code{calc_measures} returns an \code{evals} S3 object that
...
#' calc_measures(cmats)
#' calc_measures(scores = P10N10$scores, obslabs = P10N10$labels)
calc_measures <- function(cmats, scores = NULL, obslabs = NULL, ...) {
  # === Validate input arguments ===
  # Create cmats from scores and obslabs if it's missing
  ...
  evals <- calc_basic_measures(cmats[["pos_num"]], cmats[["neg_num"]], cmats[["tp"]],
