...
### S3 generics
The R language specifies S3 objects and S3 generic functions as part of the most basic object-oriented system in R. The `precrec` package provides eight S3 generics for the S3 object 
...
The `auc_ci` function calculates confidenc intervals of the calcualted ROCs by the `evalmod` function.

### Default CI calculation with normal distribution and alpha=0.05
The `auc_ci` function calculates CIs for both ROC and precision-recall AUCs. The specified data must contain multiple datasets, such as cross-valudation data.
...
The `auc_ci` function accepts a different signifiant level.
...
The `auc_ci` function accepts both normal and t-distiubtions for CI calculation.
