...
### S3 generics
The `precrec` package provides seven S3 generics for the S3 object 
...
#### Examples of the `as.data.frame` function
...
### S3 generics
All the S3 generics offered by this package are also effective for the S3 object generated by this approach.
```{r, fig.width=7, fig.show='hold'}
...
### S3 generics
All the S3 generics except for the `auc` and the `part` functions are also effective 
...
## 7. Partial AUCs
The `part` function is an S3 generic that calculates partial AUCs of both 
ROC and precision-recall curves.

### partial AUC calculations
It requires an S3 object produced by the `evalmod` function. It uses `xlim` and
`ylim` to specify the partial area of your choice.
```{r}
# Calculate ROC and Precision-Recall curves
...
### S3 generics
All the S3 generics are also effective for the S3 object generated by this approach.
```{r, fig.width=7, fig.show='hold'}
# Show ROC and Precision-Recall curves
autoplot(curves.part)
```

## 8. Simulation with balanced and imbalanced datasets
