...
### Additional measures
Eight more measures are available on request. They are the ones `ROCR`
...
The odds ratio is `NA` at the top and the bottom of every dataset, where the
2x2 table has an empty cell and the ratio is undefined. `ROCR` reports an
infinity there; `precrec` reports `NA`, as it already does for the
undefined end of precision and of the negative predictive value.
