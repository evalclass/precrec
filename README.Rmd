...
`precrec` evaluates a dataset with more than two classes by one\-vs\-rest decomposition. Each class becomes its own binary problem, so the accurate precision-recall calculations apply to it unchanged, and `auc` reports the per-class scores together with their macro-average.
...
Moreover, the `precrec` package provides ten S3 generics for the S3 objects created by the `evalmod` and `metric_curve` functions. **N.B.** The R language specifies S3 objects and S3 generic functions as part of the most basic object-oriented system in R.
